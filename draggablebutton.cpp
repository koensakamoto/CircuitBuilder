    ...
    gate(gate), gateType(gateType), onIcon(":/GATES/INPUT-ON.png"), offIcon(":/GATES/INPUT-OFF.png"), isDelete(false)
{
    ...
    // Set object names for port buttons to prevent style inheritance
    input1.setObjectName("portInput1");
    input2.setObjectName("portInput2");
    output.setObjectName("portOutput");
    ...
    // Port sizes: all 8x8px circular dots
    input1.setFixedSize(8, 8);
    input2.setFixedSize(8, 8);
    output.setFixedSize(8, 8);

    // Position ports on gate edges
    input1.move(-2, 9);
    input2.move(-2, 33);
    output.move(44, 21);
    ...
    if (gateType == INVERTER){
        input2.hide();
        input1.move(-2, 21);  // Center vertically for single input
        ...
        input1.hide();
        input2.hide();
        // Output port already styled above, just position and show
        output.move(44, 21);
        output.show();
        output.raise();
        connect(&output, &QPushButton::clicked, this, &DraggableButton::outputClicked);
        ...
        input2.hide();
        output.hide();
        // Input port already styled above, just position and show
        input1.move(-2, 21);
        input1.show();
        input1.raise();
        connect(&input1, &QPushButton::clicked, this, &DraggableButton::input1Clicked);
